...
class CostCenter extends Model
{
    ...
    protected $table = 'lainnya__cost_center';
    ...
    public static function getCostCenter($request)
    {

        try {

            $get = DB::table('lainnya__cost_center');
    ...
    public static function getParam($query)
    {
        ...
        if (isset($request['id'])) {
            ...
            if (is_array($explodeId) && count($explodeId) > 0) {
                $query = $query->whereIn('lainnya__cost_center.id', $explodeId);
            }
            // kalo param 'id' nya cuma 1
            else {

                $query = $query->where('lainnya__cost_center.id', $request['id']);
            }
        }
