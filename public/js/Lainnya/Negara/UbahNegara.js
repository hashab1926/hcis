...
$('#toolbar-ubah').click(function (evt) {
    if (checkedBox.length <= 0) {
        warningMessage('Pesan', 'pilih negara terlebih dahulu');
        return false;
    }

    const data = checkedBox.join(',');
    document.location = `${baseUrl}/negara/ubah/${encode(data)}`;
})


// begitu tombol disimpan/ update
$('#jenisfasilitas-updatestore').submit(function (evt) {
    evt.preventDefault();
